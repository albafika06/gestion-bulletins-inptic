...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="title-cell">
                <div class="bulletin-title">Bulletin de Notes — Semestre 6</div>
    ...
    <table class="notes-table">
        ...
        <tbody>
            ...
            <tr>
                <td class="libelle">
                    {{ $matiere->libelle }}
                    @if($mm && $mm->rattrapage_utilise)<em style="color:#e65100;"> (Ratt.)</em>@endif
                </td>
                ...
                <td class="{{ $couleur }}">{{ $moy !== null ? number_format($moy, 2) : '—' }}</td>
            ...
            <tr class="moyenne-row">
                <td class="libelle" style="padding-left:10px;"><strong>Moyenne {{ $ue->code }}</strong></td>
                <td><strong>{{ $ue->matieres->sum('coefficient') }}</strong></td>
                <td><strong>{{ $moyUE ? $moyUE->credits_ue : '—' }}</strong></td>
                <td colspan="3"></td>
                <td>
            ...
            <tr>
                ...
                <td colspan="3" style="color:#c62828; text-align:center;">-{{ number_format($totalAbsences * 0.01, 2) }} pt</td>
                <td colspan="4"></td>
            </tr>
            @endif
        </tbody>
    ...
    <div class="section-title">Statistiques du Semestre 6 — Promotion {{ $annee }}</div>
    <table class="stats-table">
        ...
        <tbody>
            <tr>
                <td class="label">Semestre 6</td>
                <td>
                    @if($resultat && $resultat->moyenne_semestre !== null)
                        @php $ms = $resultat->moyenne_semestre; $c = $ms >= 10 ? 'moy-vert' : ($ms >= 6 ? 'moy-jaune' : 'moy-rouge'); @endphp
                        <strong class="{{ $c }}">{{ number_format($ms, 2) }}</strong>
                    @else —
                    @endif
                </td>
                <td>{{ $resultat && $resultat->stat_moyenne_classe ? number_format($resultat->stat_moyenne_classe, 2) : '—' }}</td>
                <td>{{ $resultat && $resultat->stat_min ? number_format($resultat->stat_min, 2) : '—' }}</td>
                <td>{{ $resultat && $resultat->stat_max ? number_format($resultat->stat_max, 2) : '—' }}</td>
                <td><strong>{{ $resultat && $resultat->rang ? $resultat->rang . 'e / ' . $nbEtudiants : 'N/A' }}</strong></td>
                <td>{{ $nbEtudiants }}</td>
            </tr>
        </tbody>
    </table>
        ...
        <tr>
            ...
            <td style="width:30%;">
                @if($resultat && $resultat->moyenne_semestre >= 10)
                    @php $ms = $resultat->moyenne_semestre; $mention = $ms >= 16 ? 'Très Bien' : ($ms >= 14 ? 'Bien' : ($ms >= 12 ? 'Assez Bien' : 'Passable')); @endphp
                    <strong>{{ $mention }}</strong>
                @else <span style="color:#c62828;">—</span>
                @endif
            </td>
            <td class="label" style="width:20%;">Crédits validés S6</td>
            <td style="width:30%;">
                <strong>{{ $resultat ? $resultat->credits_acquis : 0 }} / 30</strong>
                @if($resultat) — <em>{{ $resultat->valide ? 'Semestre validé' : 'Semestre non validé' }}</em> @endif
            </td>
        </tr>
    </table>
    ...
    <table class="credits-table">
        ...
        <tbody>
            ...
            @php $moyUE = $moyennesUEs[$ue->id] ?? null; @endphp
            <tr>
                <td style="text-align:left; padding-left:5px;">{{ $ue->code }} — {{ $ue->libelle }}</td>
                <td>
                    @if($moyUE && $moyUE->moyenne_ue !== null)
                        @php $c = $moyUE->moyenne_ue >= 10 ? 'moy-vert' : ($moyUE->moyenne_ue >= 6 ? 'moy-jaune' : 'moy-rouge'); @endphp
                ...
                <td>
                    @if($moyUE)
                        @if($moyUE->statut == 'ACQUISE') <span class="moy-vert">✓ Acquise</span>
                        @elseif($moyUE->statut == 'COMPENSEE') <span class="moy-jaune">~ Acquise par compensation</span>
                        @else <span class="moy-rouge">✗ Non acquise</span>
                        @endif
                    @else —
                    @endif
                </td>
    ...
    <div class="decision-box">
        Décision du Jury — Semestre 6 :
        @if($resultat)
            @if($resultat->valide) <span class="moy-vert">Semestre 6 VALIDÉ</span>
            @else <span class="moy-rouge">Semestre 6 NON VALIDÉ</span>
            @endif
        @else En attente de délibération
        @endif
    </div>

    <table class="signature-table">
        <tr>
            <td style="width:50%; text-align:left;">Fait à Libreville, le {{ \Carbon\Carbon::now()->isoFormat('D MMMM YYYY') }}</td>
            <td style="width:50%; text-align:center;">LE DIRECTEUR DES ÉTUDES ET DE LA PÉDAGOGIE</td>
        </tr>
        <tr>
            <td></td>
            <td style="padding-top:40px; text-align:center;"><strong>{{ config('app.directeur_dep', '') }}</strong></td>
        </tr>
    </table>
